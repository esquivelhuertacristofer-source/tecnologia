...
/**
 * Base de las pantallas de entrada de la Unidad 4 (documento §5).
 *
 * Es la plantilla de oro con la única variación permitida entre unidades:
 * la ruta. La sala «Creo con la compu» tiene tres paradas; la parada del
 * CTA va de 1 a 3 (la parada 1, «Utiliza programas», ganó entrada propia
 * en la auditoría F3 — su laboratorio es el módulo 5 del port). Todo lo
 * demás — escenario Bit + video con cubrepantalla, tira de datos, fichas
 * de color pleno, CTA gigante, ruta — se conserva sin variación, que es
 * justo lo que exige la plantilla.
 */
...
const RUTA_UNIDAD4: PasoRuta[] = [
  { id: 'n1-utiliza-programas', titulo: 'Utiliza programas' },
  { id: 'n1-pinta-con-la-compu', titulo: 'Pinta con la compu' },
  { id: 'n1-guarda-tu-obra', titulo: 'Guarda tu obra de arte' },
];
...
export interface ConfigEntradaUnidad4 {
...
export function EntradaUnidad4Base({ entrada, ...props }: ActivityProps & { entrada: ConfigEntradaUnidad4 }) {
  ...
  const idxActual = RUTA_UNIDAD4.findIndex((paso) => paso.id === entrada.actividadId);
...
export default EntradaUnidad4Base;
