...
/**
 * Base de las pantallas de entrada de la Unidad 2 (documento §3).
 *
 * Es la plantilla de oro (EntradaUnidad1Base) con dos cambios de fondo: el
 * CTA abre el laboratorio propio de cada ejercicio (config.laboratorio) en
 * lugar del juego compartido de la misión, y la ruta de unidad es la de U2
 * — seis paradas. El número gigante del CTA es el número de parada en la
 * ruta (1–6; la parada 1, «Explora EduOS», ganó entrada propia en la
 * auditoría F3 — su laboratorio es el módulo 4 del port). Todo lo demás —
 * escenario Bit + video con cubrepantalla, tira de datos, fichas de color
 * pleno, CTA gigante, ruta — se conserva sin variación, que es justo lo
...
const RUTA_UNIDAD2: PasoRuta[] = [
  { id: 'n1-explora-eduos', titulo: 'Explora EduOS' },
  { id: 'n1-caza-clics', titulo: 'Caza clics' },
  { id: 'n1-laberinto-del-mouse', titulo: 'Laberinto del mouse' },
  { id: 'n1-lluvia-de-letras', titulo: 'Lluvia de letras' },
  { id: 'n1-teclas-gigantes', titulo: 'Teclas gigantes' },
  { id: 'n1-mapa-de-flechas', titulo: 'Mapa de flechas' },
];
...
export interface ConfigEntradaUnidad2 {
  ...
  /** Número de parada en la ruta de la unidad (1–6); es el número gigante del CTA. */
  parada: number;
  globo: string;
  arranqueSub: string;
...
export function EntradaUnidad2Base({ entrada, ...props }: ActivityProps & { entrada: ConfigEntradaUnidad2 }) {
  ...
  const idxActual = RUTA_UNIDAD2.findIndex((paso) => paso.id === entrada.actividadId);
...
export default EntradaUnidad2Base;
